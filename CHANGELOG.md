...
### Features
- **论文库与讨论区刷新管线优化**：继续采用本地快照即时显示与后台校准；论文来源目录读取限制为 3 路并发、元数据限制为 2 路并发，讨论主题和回复按同步点使用 20 条小页增量读取，并每 24 小时全量校准一次。
- **论文库信息架构重设计**：恢复首屏主标题、说明和实时数量，把范围、搜索、年份、月份、阅读状态、排序与论文标签拆成清晰层级；论文条目按中文标题、论文原始标题、作者机构、来源日期和标签展示，并支持标签溢出计数。
- **讨论区信息架构重设计**：恢复首屏主标题、统计与发帖操作，将搜索、版块、置顶主题和普通主题重新分层；强化未读状态、作者头像、主题元信息、回复数与阅读数的视觉辨识度。
- **响应式布局**：论文库和讨论区统一使用 Outline Web 主题 token、1280px 内容画布和窄屏重排，兼容浅色与深色主题。

### Fixes
- **刷新写放大与失败冷却**：论文元数据快照从每个 API 分页完整写盘改为每个来源完成后写一次，避免千篇论文场景反复序列化和重绘；论文与讨论的阅读统计仅在请求成功后记录刷新时间，网络失败项可在下一轮继续重试。讨论回复增量使用固定时间窗口，避免刷新期间新回复被漏记或重复累计。
- **首屏顶部空白**：论文库和讨论区在桌面宽屏下不再渲染首屏无内容的 `Scene` sticky Header，使标题紧接标签栏下方；窄屏仍保留 Header 及侧栏菜单入口。
- **个人笔记侧栏层级**：个人笔记分组直接展示已映射成员目录的一级子文档，不再重复渲染姓名根文档；分组标题负责展开折叠，嵌套目录从一级开始逐层缩进。通过个人笔记打开文档时会撤销文档集树此前的自动路径展开，同时保留用户手动展开的目录，避免成员空间跟随个人笔记联动。
- **侧栏选中态与缩进**：顶部快捷入口移除浮起卡片阴影和悬停背景叠加，改用轻量底部指示线表示唯一当前入口；个人笔记图标行使用稳定的 12px 起始缩进，每深入一级增加 16px，修复二级目录反而左移的问题。

### Design Rationale
- 刷新过程以集合目录树作为成员关系和分类的权威来源，以分页 API 只补作者、时间和论文元数据；因此增量读取仍能立即移除已移出目录的条目，不以一致性换速度。
- 参考历史版本保留高信息密度和快速筛选能力，同时继续复用正式的 Outline Web `Scene`、路由、Store、API 与缓存，不恢复旧 renderer。
- 页面首屏标题与滚动后的 `Scene` 标题各司其职：首屏负责建立页面层级，滚动标题负责持续定位，避免初始页面只剩一行说明文字。

### Notes & Caveats
- 讨论区“阅读数”延续旧版定义为去重阅读人数（`views.list` 记录数）；论文库热度延续当前正式 Web 扩展的累计浏览次数定义。
- 论文库新增的年份和月份筛选完全在已缓存的论文索引上执行，不增加网络请求；原有增量刷新、互动数据和文档打开路径保持不变。

## [1.19.0] - 2026-08-12

### Unreleased

- **个人空间恢复成员目录映射**：对照旧版 `personalRootDocId + personalRootCollectionId` 设计，将左上入口改为“个人空间”，只读取 profile 指定的成员目录及其子树，不再按创建者扫描整个工作区；新增完整目录路径、自然编号排序、按需展开、新建子文档、自动按姓名定位与手动更换目录，并采用本地快照即时显示、后台刷新。
- **简体中文界面修复**：桌面打包阶段补齐官方 Outline Web 的全部 i18n 词典，并按其 `/locales/:lng.json` 请求格式生成资源；修复用户已选择 `zh_CN`、启动环境也是中文，但安装包因缺少翻译文件而继续显示英文原文的问题。
- **论文库与讨论区缓存提速**：讨论主题索引、论文阅读量和分区统计均按 workspace 持久化到本地，页面改为 cache-first 后台同步；论文元数据使用 20 篇小页按上次同步点增量刷新，阅读量每次最多限流刷新 24 篇；讨论回复数由每主题 N+1 请求改为一次集合级评论聚合，阅读数分批轮换更新并逐批落盘。
- **窗口交互**：桌面端顶部标题栏支持双击在最大化与还原之间切换。
- **论文库资料完整性**：对照旧版恢复中英文双标题、作者/机构、会议、可点击标签、共享点赞数及原文/代码入口；元数据改为按集合分页扫描，并修复空缓存、手动刷新失效和 `views.list` 响应解析错误。
- **讨论区信息密度**：主题列表恢复真实作者与头像、相对时间、版块、未读回复提示、回复数、阅读数和 WebDAV 共享点赞数；主题资料改为分页加载完整文档记录，统计采用限流与本地缓存。
- **工作区 UI 二次收敛**：标签栏改为紧凑、扁平的文档栏，移除突出的白色卡片；论文库与讨论区合并搜索/筛选工具栏、去除重复标题和统计，并让讨论版块采用官方下划线标签。修复新文档标签可能长期显示 `Untitled` 的问题。
- **官方 Web 视觉统一**：标签页、论文库和讨论区继续沿用 Outline Web 的 `Scene`、官方按钮、搜索框、图标和主题 token，收紧页面层级与行式信息密度，补齐窄屏布局和键盘焦点状态；未恢复旧 renderer，也未引入第二套路由或数据格式。
- **标签页交互完善**：读/编辑文档共用稳定 tab identity，标签支持键盘切换、独立关闭、右键固定/关闭操作和同组拖拽排序；关闭当前标签会回到 store 返回的邻接标签。
- **论文库与讨论区重排**：论文库新增清晰的范围、标签、阅读状态和排序工具栏，讨论区新增版块计数、快速发帖面板、置顶/点赞/打开动作与缺失集合提示；原有缓存、浏览记录、互动文件、pins API 和 `openInTab` 路径保持不变。
...
### Features
- **评论面板按网页版重排**：面板 300 → 380px（300px 下 14px 正文一行放不下几个字）；正文由 13px 灰字改为 14px 正文色；时间改相对时间（「25 分钟前」，hover 看绝对时间）；编辑/删除改为悬停才出现，不再挤占姓名行；每条线程做成独立浅色卡片；锚定原文单独渲染成引用块（客户端评论会把开头的 `「…」` 行提出来，层次与网页版一致）。
- **修掉锚定原文的两行截断**：`.comment-anchor-quote` 上的 `-webkit-line-clamp: 2` 会把超过两行的引文直接切掉——这是「评论显示不完整」的一个真实来源。改为最多 6 行后内部滚动。
- **评论正文改为富文本渲染**：评论存的是 ProseMirror doc，客户端过去只做「递归取 `text`」的拍平，导致列表符号、加粗/斜体/链接、代码块围栏全部丢失，`br` / image / mention 这类没有 `text` 的节点直接产出空串，块与块之间也不分隔——web 上排好版的评论到客户端就读不成句子。新增 `commentDoc.ts` 做 ProseMirror → markdown 转换（段落/标题/有序无序列表/待办/引用/代码块/分隔线/硬换行/图片/提及 + 加粗/斜体/行内代码/删除线/高亮/链接），再交给共享 `MarkdownRenderer`，与文档正文同一套渲染。未知节点递归取内容，宁可样式退化也不丢字。
- **侧栏评论输入框改为点击展开**（对齐网页版）：默认只显示一行「写下评论…」占位，点击才展开输入框 + 发表/取消；发表后自动收回，Esc 在草稿为空时收起。从正文选中文字点「评论」带引用进来时自动展开。论坛内联流（讨论区）保持输入框常驻，那里回帖是主要动作。
- 随记板块改为 **flomo/memos 式双栏布局**（参考图对齐）：**右栏** = 大数字统计（笔记 / 标签 / 天）+ 热力图 + 标签导航（「全部笔记」清除筛选 + 「全部标签」，默认折叠前 8 个、「展开全部 N 个」切换，按出现次数排序、点击筛选）；左主栏 = 顶部条（筛选态标题 + 搜索 + 管理/回收站/导出）+ 输入框 + 时间线。右栏 sticky、独立滚动，窄屏（<900px）自动堆叠。
- 随记卡片正文改用共享 **MarkdownRenderer**（markdown-it）渲染：修复此前 `**加粗**`、`[文字](链接)`、`- 列表` 等 markdown 显示为字面符号的问题，与文档/quiz 卡片渲染一致（含代码高亮 / KaTeX / mermaid / ==高亮==）。`#标签` 仍渲染为可点击 chip 并触发筛选。
- 输入框加参考图式工具栏（视觉，按钮接现有能力）：`#` 光标处插入标签、`@` 打开文档选择器关联；插图 / Aa / 列表为「即将支持」占位；右侧发送按钮（⌘/Ctrl+Enter）。
- 卡片宽度：展示态与内联编辑态卡片、顶部速记框统一封顶、左对齐、整列一致；进入编辑不改变页宽（避免双击编辑时布局跳动）。
- **页宽可调**：顶栏加「窄 / 标准 / 宽 / 满」分段控件（560 / 680 / 840 / 铺满主栏），点选即用，`localStorage` 记忆（`notes.pageWidth.v1`）；通过 `--nt-card-w` CSS 变量驱动卡片封顶宽度。**默认档位为「宽」**（首次使用 / 未记忆时）。
- **正文字号与文档阅读区对齐**：随记卡片正文与输入框由 14px/1.75 改为 `--text-md`（16px）/1.6 + `letter-spacing: 0.01em`，与 `.document-body` 一致；代码块不跟随字距。
- **侧栏移回左侧**：统计 / 热力图 / 标签导航回到左边（DOM 顺序前置，宽屏无需 `order`）；窄屏堆叠时用 `order` 把它沉到时间线下方。内容区维持单列。
- **响应式改用 container query**：右栏是否堆叠按「随记内容区实际宽度」（`container-type: inline-size`，阈值 820px）判断，而非视口宽——修复全局侧边栏 + 右栏同时挤占时阅读区过窄、右栏被推出屏幕的问题。
- **双击编辑**：双击卡片正文直接进入内联编辑（点在链接/标签/按钮上不触发），无需悬停找「编辑」按钮。
- **编辑框自适应高度**：textarea 随内容 auto-grow（`useLayoutEffect` 按 `scrollHeight` 撑高，封顶 60vh，超出才内部滚动），编辑长笔记不再挤在固定小框里滚动；空的顶部速记框仍保持紧凑（min-height）。
...
- **富文本评论禁用「编辑」按钮**：编辑走的是「纯文本 → 每行一个 paragraph」重建（`buildCommentDoc`），只能无损承载纯段落 + 「…」引用行。评论正文改成富文本显示后，若仍允许编辑，保存会把列表/加粗/代码块静默碾平——与 v1.11.x 表格序列化那次数据丢失同类。故 `isRichComment` 识别后禁用并提示去网页版改，不做「看起来能编辑、一存就毁」。
- 内容区**试过双栏后回退到单列**：双栏下可用宽约 950px，两列各仅 ~410px，配合 16px 正文一行不到 25 个汉字；且 grid 双栏在同行卡片高度不等时底部参差（原生 masonry 在 Electron 42 的 Chromium 里仍在 flag 后面）。单列 + 页宽预设的组合更耐看。
- 窄屏堆叠时用 `order` 把左栏沉到底部：统计 + 热力图 + 标签列表是一大块，压在上方会把随记挤出首屏。
- `#标签` 可点击但又要走完整 markdown：给 `MarkdownRenderer` 加可选 `onTagClick`，仅随记启用时把 `#tag` 预处理成锚点链接 `[#tag](#nt-tag-…)`、点击拦截回调；文档渲染路径零影响（`tagsEnabled` 布尔进 useMemo 依赖，避免回调身份变化重算）。
- CommonMark 下 `#标签`（# 紧跟非空白）不构成 ATX 标题，故直接喂给 markdown-it 安全，无需改动共享渲染器的标题规则。
- 待办（TodoRow）仍复用 `renderContent` 做紧凑单行 chip+URL 渲染，未改；随记卡片走块级 markdown，两条路径按场景分工。

### Notes & Caveats
- 「天」= 有记录的天数（distinct active days，即热力图有格子的天数）；连续天数（streak）移到该数字 tooltip。
- 标签导航为简单版：单一列表、无置顶分区、无每标签自定义 emoji（本轮按需裁剪）；默认折叠前 8 个，避免标签过多撑长右栏。
- 热力图 flex 塌陷修复：右栏为 `flex column + max-height`，热力图容器带 `overflow-x:auto` 会使 `min-height` 归 0、被 flex-shrink 压到高度 0（表现为「热力图消失」）；给 `.nt-heatmap` 加 `flex-shrink:0` 固定，由右栏自身 `overflow-y` 处理溢出。
- 评论渲染新增单测 14 例（`__tests__/commentDoc.test.ts`）覆盖列表/代码块/硬换行/图片/未知节点/异常输入；本机 shell 连不上 Outline 服务器（直连与代理均超时），故未取线上真实 doc 做端到端比对，节点类型按 Outline 的 PM schema 命名（含 `bullet_list`/`bulletList` 两种写法兜底）。
- 富文本评论在客户端**不能编辑**（按钮置灰并说明原因），需到网页版改；纯文本评论编辑不受影响。
- 工具栏插图 / Aa / 列表仅为占位，未接功能。
- `#tag` 预处理走全局正则，理论上会误伤行内代码里的 `#word`（渲染成带方括号的字面文本）；随记场景罕见，暂可接受。
...
- zoom 聚焦时，在聚焦块的顶层子块上按行首 Backspace/Shift+Tab 会把该块并入/移出到（面包屑里的）zoom 根，块会移出当前视图（非数据丢失）；后续可加边界守卫。
...
### Fixes
- **修复（根因）：在客户端编辑含标记的表格会损坏单元格。** 表格序列化器逐格捕获时复用同一个 prosemirror-markdown 状态并回退 `out`，破坏了 mark 的延迟闭合记账——一个带 `**粗体**`/`==高亮==` 的单元格会把定界符泄漏进同行下一格并吃掉其内容（`| **x** | arXiv:1234 |` → `| **x** | arXiv**:34 |`）。论文库的 `| 属性 | 详情 |` 表头正是这种，在客户端一编辑就会中招。改为**每个单元格用独立的序列化子状态**渲染，标记在格内自开自闭、绝不跨 `|` 泄漏；顺带把单元格内换行统一为 `<br>`（修掉 `[hardBreak]`、`\`+换行 等残留）。CDP 往返验证：粗体/高亮/换行/多行表格全部无损。
- 注：此修复防止**今后**编辑损坏表格；历史上被旧版本压平的个别表格（整表塌成一行）需单独重建源数据。

## [1.13.2] - 2026-07-24

### Fixes
- **修复：图片后紧跟的图注引用块显示成字面「> ...」。** Outline 导出的 markdown 会把图片和其后的引用块挤在同一行（`![img](url)> **Fig. X** 图注`）；而 `>` 只有在行首才是引用块，于是图注被当普通文字、光秃秃地显示一个 `>`（图片与图注的视觉分隔也丢了）。解读文档里每张图都是这种写法，属普遍问题。新增 `normalizeOutlineMarkdown` 在渲染/载入编辑器前把 `![img](url)>` 拆成「图片 + 空行 + 引用块」，读视图与编辑器均已接入。CDP 验证：ABot-3DWorld 文档的 Fig.1/2/5/6/11/12 图注恢复为正常引用块，不再出现字面 `>`。正确分隔的内容（`)` 与 `>` 间本就有换行）不受影响。

## [1.13.1] - 2026-07-24

### Fixes
- **修复：精选专题里「建在另一篇论文下面」的论文在论文库中不可见、搜不到（影响 37 篇真论文）。** `collectFeatured`/`collectInternalWork` 原本在遇到 📖 论文后跳过其全部子文档（本意跳过附录），但用户常把新论文直接建在另一篇 📖 之下，这些真论文被一起漏掉。改为收下该论文后继续递归其子树捞取嵌套的 📖 论文（附录非 📖，不受影响），topic 沿用最近的非-📖 专题文件夹。CDP 验证：精选专题收集数 392→429，VLN-R1/Uni-NaVid/NaVid 等此前消失的论文恢复可见可搜。

### Notes
- 论文库的集合树按会话缓存、无自动刷新，新解读的论文需重开 app（或树查询过期后重挂载）才会出现——与本 bug 无关，但同样会造成「刚解读完暂时搜不到」，重启即可。

## [1.13.0] - 2026-07-22

### Features
- **新增「待办」功能。** 侧边栏「待办」入口，仅个人可见，存自己的 WebDAV 私有文件（`待办/<userId>.json`）+ 本地镜像。
  - 快速添加：文本 + 截止日期 + 优先级(高/中/低) + 行内 `#标签`，`⌘/Ctrl+Enter` 添加。
  - **智能分组**：已逾期 / 今天 / 即将(7 天内) / 以后 / 无期限；组内按优先级→截止排序；已完成折叠区。
  - 勾选完成、原地编辑、软删除 + 回收站(30 天清理)、批量完成/删除、清除已完成、导出为 Markdown(带 `- [ ]`/`- [x]`)。
  - **关联 Outline 文档**：添加/编辑可挂文档链接；读文档时头部「＋待办」一键建待办并预关联本文；行内出处 chip 点击在应用内打开。
  - 按标签/优先级/关键词筛选。

### Design Rationale
- 抽取通用 `useWebdavStore`（泛型 WebDAV JSON 存储 + 本地镜像 + 多设备安全合并），随记与待办共用，消除重复存储逻辑。
- 复用随记的 DocPicker 与标签解析；沿用软删除+回收站的数据安全范式。

### Notes & Caveats
- 截止日期仅用于分组/排序，本版不做到期桌面提醒。
- 与随记一致：不含图片；关联文档存标题快照（改名后用 docId 跳转仍正确）。

## [1.12.0] - 2026-07-22

### Features
- **新增「随记」功能（flomo 风格速记本）。** 侧边栏「随记」入口，仅个人可见，独立于 Outline 知识库，数据存自己的 WebDAV 私有文件（`随记/<userId>.json`）+ 本地镜像秒开。
  - 顶部速记框，`⌘/Ctrl+Enter` 即存即显；轻量 markdown + 行内 `#标签`（chip 可点筛选）。
  - **flomo 式贡献热力图**：近 13 周，按当天条数分 4 档绿，月份标注，hover 提示，点格子筛选当天。
  - **关联 Outline 文档**：速记框内可搜索/从「最近浏览」挑文档关联；读文档时头部「＋随记」按钮一键记录并自动预关联本文；卡片底部展示出处 chip，点击在应用内打开。
  - 标签栏 + 全文搜索；卡片支持编辑 / 置顶 / 复制。
  - **软删除 + 回收站**（30 天自动清理，可恢复/彻底删除），防云端误删。
  - 管理模式批量删除/置顶；一键导出全部随记为 Markdown。
  - **多设备安全**：每次写入前 re-GET 远端按 id 合并，避免覆盖其他设备的新随记（last-write-wins）。

### Design Rationale
- 复用现成 `webdav:get/put` IPC 通道，主进程零改动；沿用讨论区/论文库的「读改写串行链 + localStorage 镜像」离线优先范式。
- 仅个人可见 → 每人固定单文件，天然规避跨人写冲突与 WebDAV 无 PROPFIND 的限制。

### Notes & Caveats
- v1 不含图片（当前 WebDAV 通道只传文本）。
- 关联文档存标题快照，源文档改名后 chip 标题可能过期，但用 docId 跳转始终正确。
- 单文件全量重写，实验室规模（数百~数千条）完全够用；超大再迁移按月分片（已留 version 字段）。

## [1.11.41] - 2026-07-21

### Fixes
- **修复：网页版做的高亮，在客户端编辑器里显示成字面 `==文本==`。** 阅读视图本就用 markdown-it-mark 正确渲染高亮，但编辑器的 Highlight 只做了序列化（→`==`）、没做解析，导致加载含 `==高亮==` 的文档时原样显示。现在给编辑器的 markdown 解析器也挂上 markdown-it-mark（`==` → `<mark>` → 高亮标记），并在文档含 `==` 时重解析一次。已 CDP 验证：`==文本==` 正确变成高亮，且往返回 markdown 仍是 `==…==` 不丢数据。

## [1.11.40] - 2026-07-21

### Changed
- **正文排版加大间距，阅读更轻松。** 文档正文行距 1.5→1.8、字距 0→0.02em、段落间距 0.6em→0.8em、列表项 0.2em→0.3em；代码块保持等宽紧凑不加字距。编辑器、阅读视图与讨论区正文同步生效（均基于 .document-body）。

## [1.11.39] - 2026-07-21

### Changed
- **讨论区改为按「发表时间」排序（新帖在前）。** 此前按「最后活动」（帖子更新时间或最新回复）排序，导致被人编辑/回复过的老帖会冒到前面（如被组员编辑过的「欢迎来到讨论区」）。现改为按帖子创建时间倒序，新发的帖永远在前、老帖不再因编辑上浮；行内时间文案相应改为「发表于 X」。（未读高亮仍按最后活动判断，不受影响。）

## [1.11.38] - 2026-07-20

### Fixes
- **修复文档开头的空行删不掉。** 当文档第一个块是空段落（如标题下方紧跟一个空行、再接引用块/表格）时，光标停在这个空行上按 Backspace 无任何反应——因为它前面没有可合并的节点。现在在这种「文档首个空段落」上按 Backspace 会删除该空行，让后面的内容上移。（已 CDP 复现并验证：首行空段落可删；开头有文字的段落、非首行空段落等正常编辑行为不受影响。）

## [1.11.37] - 2026-07-20

### Fixes
- **修复评论头部在窄面板里换行错乱。** 加了「编辑」按钮后，评论的「头像·姓名·时间·编辑·删除」一行在窄评论面板里挤到换行（姓名被逐字断行、编辑/删除上下堆叠）。现在：姓名不换行、时间过长时省略、编辑/删除固定靠右单行排列，时间戳去掉秒（如「2026/7/20 14:16」）更省空间。

## [1.11.36] - 2026-07-20

### Features
- **阅读页新增浏览者头像行 + 统计信息面板（对齐网页版）。** 📖 论文标题下的 byline 现在像网页版一样显示浏览者头像（重叠排列，超过 5 个显示「+N」）。头部新增「统计信息」按钮，打开面板可看：源（创建时间、最后更新时间与人）、统计信息（约几分钟阅读、字数、字符数、表情符号数）、贡献者（创建者 / 最近编辑，及「另有 N 人参与编辑」）、浏览者头像。
- **表格插入按钮改为清晰的蓝底白「+」圆钮。** 原来行/列边缘的插入符号太细太淡，现替换为网页版那种醒目的蓝色圆形「+」（放大、加阴影）。

### Changed
- **侧边栏导航图标略微放大**（18px → 20px），更好点。

## [1.11.35] - 2026-07-20

### Changed
- **阅读量改为「去重人数」（有几个人看过），不再是总浏览次数。** 论文库列表、论文阅读页、讨论区帖子三处的浏览统计，由「累加每人的浏览次数」改为「浏览过的去重人数」，文案统一为「N 人读过」（排序项相应改为「阅读人数」）。数据仍来自 Outline 服务器 `views.list`（网页端 + 桌面端统一记录、全组共享），只是口径从「次数」换成「人数」。浏览缓存键已升级（v1→v2），装新版后会重新按人数统计。

## [1.11.34] - 2026-07-20

### Fixes
- **修复：修改文档/讨论区帖子标题后，重新打开又变回旧标题。** 标题改动其实已保存到服务器，但保存后没有同步文档详情的本地缓存，而标题输入框只在打开时用缓存里的旧标题初始化一次 → 重开显示旧标题、看起来"没生效"。现在保存成功后会把新标题/正文写回详情缓存，重开即显示最新（已 CDP 复现并验证修复）。

## [1.11.33] - 2026-07-19

### Changed
- **表格操作菜单重做，对齐网页版。** 去掉常驻在表格下方、塞满文字按钮的旧菜单。现在：插入行/列用行列边缘的 ⊕ 符号（悬停出现）；点击行/列的灰色 grip 选中后，才弹出一个干净的竖直菜单（切换表头、在前/后插入、合并/拆分单元格、删除此行/列、删除整个表格），菜单按选中的是行还是列自适应。光标只是在单元格里编辑时不再弹任何菜单。

## [1.11.32] - 2026-07-19

### Features
- **侧边栏支持拖拽调整笔记顺序（对齐网页版）。** 在侧边栏直接拖动文档即可重排：拖到某条上/下缘＝排到它前/后，拖到中间＝作为其子文档。拖动时显示插入位置指示线，松手即乐观更新并通过 `documents.move` 持久化（整棵子树跟随移动）；失败自动回滚。索引语义与 Outline 网页版一致（放到目标前＝目标原位置，放到后＝＋1），已 CDP 端到端实测重排后刷新仍保持。按字母排序的集合不参与手动拖拽（与网页版一致）。

## [1.11.31] - 2026-07-18

### Features
- **论文库新增「我赞过 / 最近浏览」快速切换，方便回找。** 论文库顶部加一组切换：全部 · 我赞过 · 最近浏览（各带数量）。「我赞过」按当前用户的点赞过滤（与阅读页/列表点赞同源）；「最近浏览」调 Outline `documents.viewed` 取本人浏览过的论文，按浏览时间倒序排列，打开任意论文即自动记录。搜索、领域、状态筛选在这三个范围内都继续可用。

## [1.11.30] - 2026-07-18

### Fixes
- **论文库「更新时间」排序：新解读的论文现在正确排到最顶。** 之前排序键取自 `usePaperMetas`（元数据 10 分钟才刷新），刚解读的论文节点已出现、但还没拿到 `updatedAt`，被当作最旧沉到列表底部。改为：元数据 staleTime 10 分钟→30 秒（打开论文库即后台刷新，新论文数秒内拿到真实时间冒到顶），且无元数据的论文排在末尾而非顶部（避免少数 documents.list 分页够不到的「精选论文」旧文档抢占最新位置）。

### Changed
- **阅读页互动区重做：去掉五星评分，点赞 ＋ 浏览次数移到标题下方。** 📖 论文阅读页在标题正下方显示一行「N 次浏览 · 👍 N」（CSDN 风格 byline），点赞仍与论文库列表共享同一份 WebDAV 数据、双向同步；原右上角的点赞/评分条移除，右上角浏览量对论文页也不再重复显示。

## [1.11.29] - 2026-07-18

### Changed
- **「关系图」入口移到侧边栏右下角。** 从顶部快捷导航行移出，改为固定在侧边栏底部账户行右侧的角落图标。
- **关系图默认「只看有连接的」。** 只有近期解读会在正文里互相内链，多数论文没有跨引用、呈孤立点；默认隐藏孤立节点，只显示有引用关系的论文网络（约 119 篇 / 142 条边），避免「满屏空点」。取消勾选可看全部。

### Removed
- **删除「📦 Cubox 历史回补清单」索引页**（移入回收站，可恢复）。它此前作为一个高连接度枢纽出现在论文库与关系图里。
